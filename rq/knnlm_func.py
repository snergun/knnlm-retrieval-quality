...
def run_eval_ppl(context, validation_split=False):
    ...
    if ext_lm_modified_prob is not None:
        print("Using external LM modified probabilities")
    ...
    dev_probs += (lm_modified_prob,) if ext_lm_modified_prob is not None else ()
    dev_min_dist = (-1 * dists).min(-1)
    # Find optimal coeffs and b if not provided
    if coeffs is None:
        ...
        best_b = len(coeffs)
        best_coeffs = coeffs
        log_progress(f"Using provided coefficients: {best_coeffs}")
    ...
    # Save combined probabilities to file
    os.makedirs("rq/results", exist_ok=True)
    with open(f"rq/results/knn_lm_{split}_probs.txt", "w") as f:
        for prob in dev_combined_probs:
            f.write(f"{math.exp(prob.item())}\n")
    dev_ppl = eval_ppl(dev_combined_probs)
    log_progress(f"Number of buckets: {best_b}, Full Dev PPL: {dev_ppl:.4f}")        
